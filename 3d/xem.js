...
const MA = Q.get("m") || document.body.dataset.moHinh || "thap_ong_khoi";
const TRANG = "3d:" + MA;
...
/* ── dựng cảnh ─────────────────────────────────────────────────────────── */
const canvas = $("c");
const renderer = new THREE.WebGLRenderer({ canvas, antialias: true, preserveDrawingBuffer: true, stencil: true });   // stencil: tô mặt cắt
...
scene.add(new THREE.HemisphereLight(0xfbf7ee, 0x8a8478, 1.6));
...
const VIEN = new THREE.LineBasicMaterial({ color: 0x2a2a28, transparent: true, opacity: 0.28 });
const VIEN_CHON = new THREE.LineBasicMaterial({ color: 0xb8412c });
...
async function napMoHinh() {
  DM = await (await fetch("mo-hinh/danh-muc.json", { cache: "no-store" })).json();
  ...
  const kq = await Promise.all(thuTu.map((ma) => loader.loadAsync(`mo-hinh/lk/${ma}.glb?t=${Date.now()}`)
  ...
  GOC.userData = { ...extras, nhom, tieu_de: CANH.tieu_de || MA, lop_boc: CANH.lop_boc || [],
...
napMoHinh().then(() => {
  ...
  GOC.traverse((o) => {
    ...
    const u = o.userData;
    if (!u.id) return;
    if (u.vat_lieu === "khoang") o.material = VL_KHOANG;       // khoang khí: xanh nhạt gần như trong, bấm được
    ...
    const e = new THREE.LineSegments(new THREE.EdgesGeometry(o.geometry, 25), VIEN);
    e.raycast = () => {};
    o.add(e);
    o.userData.vien = e;
...
function dungNhomKetCau() {
  ...
  for (const x of DM.linh_kien) {
    ...
    cb.onchange = () => { cb.checked ? lkTat.delete(x.ma) : lkTat.add(x.ma); dungLop(GOC.userData.nhom || []); capNhatHien(); };
    ...
    chi.onclick = (e) => { e.preventDefault(); lkTat.clear(); for (const y of DM.linh_kien) if (y.ma !== x.ma) lkTat.add(y.ma); dungNhomKetCau(); dungLop(GOC.userData.nhom || []); capNhatHien(); };
    li.append(chi);
    ul.append(li);
  }
}
...
/* ── LỚP NHIỆT · GIÓ · PHƯƠNG ÁN (chỉ ở máy) ──────────────────────────────
   Dữ liệu do tools/blender/xuat_nhiet_gio.mjs sinh ra cạnh .glb. Bản site KHÔNG có tệp ấy (và
   không có nhiet.js) — thiếu tệp thì không làm gì, trang vẫn là trình xem thường. */
function moLopNhiet() {
  if (!CANH.nhiet) return;                  // cảnh không khai lớp nhiệt (vd ?m=nha) thì không hỏi tệp
  fetch(`mo-hinh/${MA}.nhiet.json`, { cache: "no-store" })
    .then((r) => (r.ok ? r.json() : null))
    .then((d) => d && import(d.loai === "mai" ? "./nhiet_mai.js" : "./nhiet.js").then((m) => m.batDau({
...
function hienDuoc(o) {
  ...
  if (lopTat.has(u.nhom)) return false;
...
function apMo() {
  for (const o of PHAN) {
    ...
    let m = goc;
    const mo = lopMo.has(o.userData.nhom) || vatMo.has(o.userData.id);
    if (mo) {
      m = VL_MO.get(goc);
      if (!m) {
    ...
    o.userData.vlHien = m;
    if (o !== chon && !THEM.has(o)) o.material = m;
...
function ban(ev, choMa = false) {
  ...
  for (const h of hits) {
    if (matCat && matCat.distanceToPoint(h.point) < 0) continue;   // phần đã bị cắt bỏ
...
$("tim").addEventListener("input", () => {
  ...
  for (const o of kq) {
    const li = document.createElement("li");
    li.textContent = o.userData.ten;
    li.onclick = () => { if (!hienDuoc(o)) { anTay.delete(o.userData.id); rieng = null; lopTat.delete(o.userData.nhom); if (lkTat.delete(o.userData.linh_kien)) dungNhomKetCau(); if (o.userData.ma) hienMa = true; dungLop(GOC.userData.nhom || []); capNhatHien(); } datChon(o); khungVua([o]); };
    ul.append(li);
  }
});
...
function vlNap(kieu) {
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      uniform vec3 uNen; uniform vec3 uMuc; uniform float uKieu; uniform float uBuoc; uniform float uPR;
      ...
      void main() {
        ...
        gl_FragColor = vec4(mix(uNen, uMuc, m), 1.0);
      }`,
...
function veGhiChu() {
  ...
  for (const b of goc) {
    ...
    nut.append(bTra, bXong); li.append(nut);
...
function denGhiChu(b) {
  if (b.neo && THEO_ID.has(b.neo)) {
    ...
    if (!hienDuoc(o)) { anTay.delete(b.neo); rieng = null; lopTat.delete(o.userData.nhom); if (o.userData.ma) hienMa = true; capNhatHien(); }
    datChon(o);
    for (const id of b.neo_them || []) {
      const k = THEO_ID.get(id);
      if (!k) continue;
      if (!hienDuoc(k)) { anTay.delete(id); lopTat.delete(k.userData.nhom); capNhatHien(); }
...
function veGhim() {
  ...
  if (nhomVungGhi) nhomVungGhi.clear();
  nhomCTGhi.clear();
  for (const b of GHI_CHU) {
    if (b.tra_loi_cho || b.da_xong) continue;
    if (Array.isArray(b.hinh)) for (const r of b.hinh) { try { nhomCTGhi.add(dungVat(r, MAU_GHI)); } catch { /* bản ghi hỏng: bỏ qua */ } }
    const g = docChuoiGoc(b.truoc);
...
function batVung() {
  ...
  thoiThem();
  vungBat = true; vungTam = null; vungXong = null; nhomVung.clear();
...
function chamVung(e) {
  ...
  nhomVung.clear(); nhomVung.add(duongVung(r.pts, 0x1d1d1b));
...
function ve() {
  ...
  if (lbDong) lbDong(performance.now());
...
window.XEM = {
  ...
  ghiChu: () => GHI_CHU,
  veMotKhung: () => { controls.update(); if (GOC) { datGhim(); datNhanThuoc(); } renderer.render(scene, camera); },
